"""
Converts the original NYU Machine learning dataset to a format 
easily accessed by fastMRI modules.

kspace data is scaled by 1e-8 
target is obtained via RSS reconstruction

Stanford 2D FSE dataset: http://mridata.org/list?project=NYU%20machine%20learning%20data 
Modified from: https://github.com/z-fabian/MRAugment/tree/master/data/stanford
"""
import os
import numpy as np
import pathlib
...
def cli_main(args):
    ...
    for i, mri_f in tqdm(zip(range(len(mri_files)), sorted(mri_files)), total=len(mri_files)):
        ...
        scaling = 1.  # scale measurements to similar range as fastMRI
