"""
Retrospectively synthesized axial views of the stanford 3D dataset.
...
def cli_main(args):
    ...
    for i, mri_f in tqdm(zip(range(len(mri_files)), sorted(mri_files)), total=len(mri_files)):
        with h5py.File(mri_f, "r") as hf:
            k = hf['kspace'][()]
            i = ifft2_np(k)
            i_ax = i.transpose(2,1,3,0)
            k_ax = fft2_np(i_ax)
            kspace = np.fft.fftshift(k_ax, axes=-1) # kspace for new view is shifted for some reason, i.e, low. freqs are at the edges
